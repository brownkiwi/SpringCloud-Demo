...
public class Test4 {
    public static void main(String[] args) {
        ...
        ResponseEntity<ResponseBox<UserinfoDTO2>> responseEntity = template.exchange("http://localhost:8080/get/test4?id=" + id + "&username=" + username + "&password=" + password + "&age=" + age + "&insertdate=" + insertdate, HttpMethod.GET, null, new ParameterizedTypeReference<ResponseBox<UserinfoDTO2>>() {
        });
        ResponseBox<UserinfoDTO2> box = responseEntity.getBody();
        UserinfoDTO2 userinfoDTO2 = box.getData();
        System.out.println(box.getResponseCode() + " " + userinfoDTO2.getId() + " " + userinfoDTO2.getUsername() + " " + userinfoDTO2.getPassword() + " " + box.getMessage());
    }
}
